...
def query_orders(page, session, sheet):
    ...
    2026/09/05（週六，非開盤日）拿已刪除的預約書號測過一次：委託查詢頁（這支函式
    走的 type=2）查無資料，預約查詢頁（type=4）查得到同一批列。**還沒排除「非開盤
    日本來就查不到」這個原因**，還沒在真正的開盤日重測，所以「開盤日當天已刪除的
    預約單透過這支查詢查不查得到」目前沒有結論——不要把那次測試當成定論。
    """
