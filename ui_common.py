...
def stock_title(label):
    ...
    更新分頁訊息框、歷程分頁都要把同一輪股數／成本兩筆事件併成一行，靠的是
...
def within(stamp, when, today):
    ...
    「歷程」分頁跟更新分頁的訊息框（見 ui_sync._fill_notes）都靠這個篩今天
...
def ask_cash_method(parent, current):
    ...
    「登入」按成它旁邊那一顆（當時兩顆疊在同一直行，而且那一顆第一次讀取前的標籤
    是「登入+讀取全部帳戶」，也帶著「登入」兩個字；2026/08/30 版面拆開、標籤改成
    「更新全部帳戶」之後這個誤按小很多），這個視窗跳出來又走不掉，最後只能把整支
    程式關掉重開。問題不在
