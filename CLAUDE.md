...
| `ui_order.py` / `ui_order_exec.py` | 下單分頁：前者收設定、讀 Excel、算執行預覽，後者是按下「開始下單」之後的依序執行引擎（吃凍結好的 queue，不管那份 queue 是哪個作業產生的），以及一輪跑完之後的三段收尾（撤零股單／查網頁持股／重讀股價判斷收斂，見 docs/介面規劃.md 9.8、9.9） |
...
- **下單多輪之間的持股一律查網頁，不從 Excel 讀**（2026/09/04，見
  ...
  一路到 `ORDER_MULTI_ROUND_CAP` 為止。輪與輪之間走 `_start_round_holdings`：查一次
  未實現損益，那份就是「還剩多少」，寫進 `order_holdings`（畫面與引擎同一份），
  網頁上查不到的那幾檔寫 0（＝賣光了）；股價另外讀 Excel I 欄，兩個來源分開。
  2026/09/03～09/04 中間那一版是「查網頁→寫回 Excel→落帳→重讀」，已經拿掉，
  連同它為了修那條路而加的 `planner.plan(zero_missing=…)`——**不要再把持股繞回
  Excel**，同一份數字轉手一次就多一次被 planner 規則吃掉的機會（真的發生過）。
  代價是**跑完之後 Excel 是舊的**，要人自己按一次更新，所以每一句結論後面都接
  `STALE_EXCEL_NOTE`，那句話不能拿掉。
- 出清零股是「掛了再撤」不是 IOC——零股那一場沒有 IOC 可選（`orders.BS_FLAG_ODD`），
  所以 6 秒後的撤單是**程式的責任**（`ODD_CANCEL_WAIT_MS`，原本 20 秒，2026/09/04
  改成 6 秒：盤中零股每 5 秒集合競價撮合一次）。要撤哪幾筆是查回來再過濾的
  （`apcode == '5'` ＋這一輪的股票＋賣出＋`open`），不是靠程式記得自己送出了什麼
  ——半自動時按確認的是人，拿不到委託書號。細節見 `docs/介面規劃.md` 9.8。
