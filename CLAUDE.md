...
- **下單每一輪開始前都要跑完整同步，第 1 輪也不例外，不能只重讀 Excel**
  ...
  **也不要改成「直接看網頁持股、省掉寫 Excel」**（2026/09/04 試過，當天改回來）：
  對出清成立，但全持股交易／買賣股票的張數與價格來自下單試算 M19:N28，那幾格是
  `自動計算` 拿 E/F/B8 算的——不寫回去，下一輪讀到的試算就還是上一輪的計畫。這一段
  的作用不是搬數字，是**讓 Excel 的公式有機會重算**，網頁給不了。
- 出清零股是「掛了再撤」不是 IOC——零股那一場沒有 IOC 可選（`orders.BS_FLAG_ODD`），
  所以幾秒後的撤單是**程式的責任**（等多久看 `ODD_CANCEL_WAIT_MS`，2026/09/04 起是
  6 秒：盤中零股每 5 秒集合競價撮合一次，規劃文件原文那個 20 秒是白等 3 個週期）。
