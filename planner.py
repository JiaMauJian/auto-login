...
def plan(sheet_data, record, book, today, method=METHOD_OPENING):
    ...
    呼叫端只有更新分頁一個（`ui_sync.replan`）。2026/09/04 之前還有第二個
    ——多輪出清輪與輪之間那次同步——它為了「程式剛剛把這一檔賣光了」帶了一個
    `zero_missing` 參數進來，讓那幾檔在網頁上消失時歸零而不是維持原樣。那條路
    同一天改成直接看網頁持股、不再經過 Excel（見 `ui_order_exec.
    _start_round_holdings`），這個參數跟著整段拿掉了：現在這裡只服務「人自己按
    更新」這一種情境，而那個情境的規矩從頭到尾沒變過——網頁沒有這一列就不動。
    ...
    for line in sheet_data["rows"]:
        ...
        if found is None:
            ...
            # 0 的列根本沒有這個矛盾——網頁本來就不該有它。人自己留著當觀察名單的
            # 空持股列就是這一種，每次更新都跳一次黃字警告＋帳戶掛 ⚠
            # （見 ui_sync._summary）只是在喊狼來了。提案照舊產出（missing=True，
            # 不會寫），只是不出警告。
...
def _cash(sheet_data, record, book, today, warnings, method=METHOD_OPENING):
    ...
    現金那一列。跟持股不同的地方全部集中在這裡。
        ...
        bank     銀行餘額 + 淨收付(T+0) + 淨收付(T+1)   快照，網頁直接給絕對值
...
def initialize(sheet_data, book, sheet_name, today, at):
    ...
    介面在「讀取」回呼裡跑一次（見 ui_background._initialize）。股數/成本
    不需要初始化 —— 程式每次都直接以網頁值覆蓋，沒有「先接管」這一步。
