...
那顆「修改」只在今天用「初始餘額累加」的時候才出現 —— 它改的是那一種算法的基準。
...
上面那個是其中一種（「初始餘額累加」）。另一種是「銀行餘額推算」：
    ...
    現金餘額 = 銀行餘額 + 還沒交割的成交淨收付

兩種並存不是備援，是各有各正確的日子：全額交割股當天，錢在成交當下就從銀行餘額
扣走、但同一筆還掛在當日淨收付上（下午才更新改正），那天只有初始餘額累加是對的；
反過來，匯撥與股利不會進淨收付，只有銀行餘額看得到。完整說明見
docs/現金餘額兩種算法.md。

它是一個總開關，管全部人（大家買賣的標的一樣，要中一起中）。工具列上只把
「今天用的是哪一種」寫出來，沒有任何入口可以改 —— 九點開盤要下單之前就已經決定
今天用哪一種了，而那正是那個視窗問的時機。這次程式開起來、第一次按「讀取網頁資料」
會先跳一個視窗問今天用哪一種 —— 問在抓資料之前，因為算法決定要不要去查銀行餘額
那兩支（20 個帳號就是 40 次往返，用初始餘額累加的日子一次都用不到）。同一次執行
裡讀第二次以後不會再問；關掉程式重開，下次讀取會再問一次（見 `cash_method_asked`，
故意只放記憶體、不記進紀錄檔，讓「重開就能換答案」不必靠還原 Excel 備份）。

程式只算選中的那一種。曾經兩種都算、在現金那一條後面寫著另一種算出來多少當對帳
...
五個區塊拆成五個 mixin，各自一個檔案：
    ...
    ui_sync.py          同步分頁：左邊名單、右邊明細、現金那一條、接管
...
class SyncApp(UiLayoutMixin, UiCertMixin, UiBackgroundMixin, UiSyncMixin, UiHistoryMixin):
    def __init__(self, root):
        ...
        self.cash_method = tk.StringVar(value=self._saved_method())
