...
手改會在下次寫入時被程式直接蓋掉。所以基準本身就寫在畫面上：現金那一條底下
...
第一次按「讀取網頁資料」會先跳一個視窗問今天用哪一種 —— 問在抓資料之前，因為
算法決定要不要去查銀行餘額那兩支（20 個帳號就是 40 次往返，用初始餘額累加的
日子一次都用不到）。同一次執行裡讀第二次以後不會再問；關掉程式重開，下次讀取
會再問一次（見 `cash_method_asked`，故意只放記憶體、不記進紀錄檔）。

問過、名字顯示出來之後，工具列上那個名字本身也是一個開關：點一下先跳確認視窗、
再換成另一種（見 `ui_background._toggle_cash_method`），不必等重開程式或還原
Excel 備份。2026/08/20 加的，原因是測試時每次都要還原備份才能換答案太重；
...
五個區塊拆成五個 mixin，各自一個檔案：
    ...
    ui_sync.py          同步分頁：左邊名單、右邊明細、現金那一條
